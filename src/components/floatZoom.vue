...
<script>
import { ref, unref, onMounted, defineComponent, getCurrentInstance, computed, inject } from 'vue';
  ...
  setup() {
    ...
    const zoom = (direction) => {
      unref(map)
        .getView()
        .animate({
          zoom: unref(map).getView().getZoom() + (direction === 'in' ? 0.5 : -0.5),
          duration: 250
        });
    };
