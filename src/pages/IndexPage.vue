...
export default defineComponent({
  ...
  setup(props) {
    ...
    const onShowDetail = (option) => {
      const { content, image, title, type = LAYER_TYPE[0], coordinate, feature_type = FEATURE_TYPE[0] } = option;
            ...
            response.forEach((r) => {
              ...
              if (isHighLight) {
                ...
                floatDetailProps.value.coordinate = toStringHDMS(
                  transform(coordinate, unref(map).getView().getProjection().getCode(), 'EPSG:4326')
                ).replace('N ', 'N\n');
                setTimeout(() => {
                  captureScreenshot().then((response) => {
                    floatDetailProps.value.image = response;
        ...
        unref(map)
          ...
          .forEach((layer) => {
            if (layer instanceof ImageLayer) {
              const url = layer
                .getSource()
                .getFeatureInfoUrl(evt.coordinate, unref(map).getView().getResolution(), 'EPSG:5899', {
