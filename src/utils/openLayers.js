...
import { Vector as VectorLayer, VectorImage as VectorImageLayer, Image, Tile as TileLayer } from 'ol/layer';
...
const clusterStyleFunction = function (feature, resolution) {
  ...
  if (size > 10 && resolution > 30) {
    ...
    features.forEach((f) => {
      ...
      if (f.getGeometry().getType() === 'MultiLineString' || f.getGeometry().getType() === 'LineString')
        widthStroke = 3;
      const defaultStyle = new Style({
        stroke: new Stroke({
          color: colorStroke,
